...
public class OrderDeleteController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        productDAO.updateProductQuantity(productId, product.getQuantityPerUnit() + deDAO.getOrderDetailsById(orderId).getQuantity());

        deDAO.deleteOrderDetails(orderId, productId);
        ...
        int type = loggedInAccount.getType();
        if (type == 2) {
            request.getRequestDispatcher("./my-order?accountId=${sessionScope.account.username").forward(request, response);
        } else {
